...
- **编辑器**：固定双栏（左右两个独立 Monaco Editor），聚焦侧有高亮边框；中间有交换/左右互传按钮；查找替换作用于聚焦侧。
...
- **查找替换面板**：位于编辑器顶部，含 查找 / 替换 / 列表 三个页签；列表页签为「聚焦编辑器 → 分割 → 自动写入另一侧」流程，分隔符支持自动检测/中英文逗号分号/顿号/空格/正则，排序规则（模板）可一键调用；文本对比即双编辑器本身，暂存区/模板条目可拖拽到编辑器。
