...
- **状态**：Zustand + `persist` 中间件，key 前缀 `ww:`；所有数据（工作区/暂存区/规则/排序模板/设置）自动保存到 localStorage。
- **编辑器**：固定双栏（左右两个独立 Monaco Editor），聚焦侧有高亮边框；中间有交换/左右互传按钮与宽度调节拖拽；查找替换作用于聚焦侧；撤销/重做/清空在标签栏左侧。
- **图标**：界面图标用 lucide-react；品牌 Logo 用 `src/assets/favicon.svg`（模块导入，两种构建都内联）。
- **路径别名**：`@/` 指向 `src/`。
- **主题**：CSS 变量（oklch）+ `.dark` 类切换，Monaco 主题跟随。
- **全局工具**：`src/tools/registry.ts` 注册表 + 左侧竖向工具栏（Photoshop 式）入口；工具是纯函数（输入文本 → 输出文本），作用于当前工作区（选区优先），编辑器内可 Ctrl+Z 撤销；新增工具只需在注册表追加一条。
- **查找替换面板**：位于编辑器顶部（独立卡片），三行紧凑布局：第一行查找+替换+按钮，第二行替换规则+分隔符+分割（聚焦编辑器 → 自动写入另一侧），第三行排序规则；替换预览直接写入另一侧编辑器。
- **工作区**：每个工作区独立保存左右两栏内容；暂存区宽度与左右分栏比例可拖动调节（记忆在设置中）；双编辑器在窄屏纵向堆叠、宽屏左右并排。
