...
- **编辑器**：固定双栏（左右两个独立 Monaco Editor），聚焦侧有高亮边框；中间有交换/左右互传按钮；查找替换作用于聚焦侧；撤销/重做/清空在标签栏左侧。
...
- **查找替换面板**：位于编辑器顶部，两个页签；「查找替换」页签为一行式（查找输入 + 替换输入 + 按钮），第二行为规则；「列表」页签第一行为分隔符与分割（聚焦编辑器 → 自动写入另一侧），第二行为排序规则；替换预览直接写入另一侧编辑器。
