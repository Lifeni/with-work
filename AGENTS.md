...
- **查找替换面板**：位于编辑器顶部，含 查找 / 替换 / 列表 三个页签；列表页签为「源文本框 → 分割/排序 → 结果文本框」流程，分隔符支持自动检测/中英文逗号分号/顿号/空格/正则，自定义排序模板可一键调用；文本对比走双编辑器（Diff），暂存区/模板条目可拖拽到编辑器。
