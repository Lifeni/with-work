...
- **查找替换面板**：位于编辑器顶部左侧（独立卡片），一行式（查找输入 + 替换输入 + 按钮）+ 规则行；替换预览直接写入另一侧编辑器。
- **列表面板**：位于编辑器顶部右侧（独立卡片），仅两行：分隔符与分割（聚焦编辑器 → 自动写入另一侧）+ 排序规则。
- **工作区**：每个工作区独立保存左右两栏内容；暂存区宽度与左右分栏比例可拖动调节（记忆在设置中）。
