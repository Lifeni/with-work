...
export const FindReplacePanel = forwardRef<FindReplaceHandle, Props>(
  function FindReplacePanel({ editor, initialMode = "find" }, ref) {
    ...
    const [preview, setPreview] = useState<{ original: string; result: string; count: number } | null>(
      null,
    );
    ...
    const content = useWorkspaceStore((s) => s.workspaces.find((w) => w.id === s.activeId)?.content ?? "");
    ...
    return (
      <div className="border-b border-border bg-muted/40 px-3 py-2">
        <div className="flex items-center gap-1.5">
          ...
          >
            {findError ? "无效" : matches.length > 0 ? `${current + 1}/${matches.length}` : find ? "0" : "—"}
          </Badge>
          ...
          <Toggle active={highlightAll} title="全部高亮" onClick={() => setHighlightAll(!highlightAll)}>
            <Highlighter />
          </Toggle>
          <Button
            variant="ghost"
        ...
        {mode === "replace" && (
          <div className="mt-1.5 flex items-center gap-1.5">
            ...
            />
            <Button size="sm" variant="secondary" className="h-7 shrink-0 text-xs" onClick={replaceOne}>
              替换
            </Button>
            <Button size="sm" variant="secondary" className="h-7 shrink-0 text-xs" onClick={replaceAll}>
              全部替换
            </Button>
